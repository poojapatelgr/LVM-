...
def isapprover(request):
    if request.user.is_authenticated:
        ...
        if user_profile.approver == True:
            ...
            return redirect('leaveman:export')
        else:
            return redirect('leaveman:new')
    else:
...
def export_filter(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                csvfile = df.to_csv(f'{tmpdir}/foo.csv', index=False)
                df = pd.read_csv(f'{tmpdir}/foo.csv', dtype={"user_id": str}, index_col=False)
                df.fillna("P", inplace=True)
                writexlsx(df,'/home/sikshana/Leaveapp/leaveman/ExportTemplate.xlsx',oxlsx=filepath)
    ...
    return render(request, 'leaveman/export.html', {'form': form})
...
def pgroups(v, **kwargs):
    ...
    row = row.astype(str)
    row['user_id'] = v.user_id.unique()[0]
    for r,d in dlist:
        for e in d:
            row[e] = r
    return row.fillna('P')
...
def exportCSV(form):
    ...
    allusers = [z.user.username for z in rlist if not z.user.is_staff if not z.user.is_superuser]
    ftdata = list(records.objects.filter(
        Q(from_date__gte = form.cleaned_data['from_date']) & Q(from_date__lte = form.cleaned_data['to_date']) & Q(user__in = allusers)).values())
    ...
    fdf.insert(1,"Employee Name",fdf.user_id.apply(lambda x: Profile.objects.get(user = x).employee_name), True)
    ndf = fdf.copy(deep=True)
    ndf.fillna("P").to_csv("/tmp/foo.csv")
    return ndf.fillna("P")
